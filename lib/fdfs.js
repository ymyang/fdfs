...
FdfsClient.prototype._getTracker = function() {
    ...
    return _getTrackerConnection(config).then(function(tracker) {
        ...
        return Promise.reduce(_.range(this._trackers.length), function(total, index) {
            if (tracker) {
                return tracker;
            }
            if (index == this._trackerIndex) {
                return;
            }
            config = this._getTrackerConfig(index);
            return _getTrackerConnection().then(function(tr) {
                tracker = tr;
            });
        }, 0);
