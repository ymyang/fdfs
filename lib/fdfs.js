...
FdfsClient.prototype._init = function() {
    ...
    this.config.trackers.forEach(function(tc) {
        tc.timeout = _self.config.timeout;
        tc.charset = _self.config.charset;
        var tracker = new Tracker(tc) ;
        _self._trackers.push(tracker);
        tracker.on('error', function(err) {
            logger.error(err);
            // 将有错误的tracker剔除
            _self._trackers.splice(_self._trackers.indexOf(tracker), 1);
            // 检查是否还有可用的tracker
            if (_self._trackers.length === 0) {
                _self.emit('error', new Error('There are no available trackers, please check your tracker config or your tracker server.'));
            }
        });
    });
};
...
FdfsClient.prototype._doUpload = function(file, options, callback) {
...
FdfsClient.prototype._upload = function(file, options, callback) {
    ...
    _normalizeUploadParams(file, options, function(err) {
        ...
        _self._doUpload(file, options, callback);
    });
};

FdfsClient.prototype._download = function(fileId, options, callback) {
...
FdfsClient.prototype._del = function(fileId, callback) {
...
FdfsClient.prototype._setMetaData = function(fileId, metaData, flag, callback) {
    if (is.function(flag)) {
        callback = flag;
        flag = 'O';
    }
...
FdfsClient.prototype._getMetaData = function(fileId, callback) {
    this._getTracker().getUpdateStorage(fileId, function(err, storage) {
        if (err) {
            callback(err);
            return;
...
FdfsClient.prototype._getFileInfo = function(fileId, callback) {
    this._getTracker().getUpdateStorage(fileId, function(err, storage) {
        if (err) {
            callback(err);
            return;
...
FdfsClient.prototype.upload = function(file, options, callback) {
    ...
    if (callback) {
        this._upload(file, options, callback);
        return;
    }

    var _self = this;
    return new Promise(function(resolve, reject) {
        _self._upload(file, options, function(err, fileId) {
            if (err) {
                reject(err);
                return;
            }
...
FdfsClient.prototype.download = function(fileId, options, callback) {
    if (callback) {
        this._download(fileId, options, callback);
        return;
    }

    var _self = this;
    return new Promise(function(resolve, reject) {
        _self._download(fileId, options, function(err) {
            if (err) {
                reject(err);
                return;
            }
...
FdfsClient.prototype.del = function(fileId, callback) {
    if (callback) {
        this._del(fileId, callback);
        return;
    }

    var _self = this;
    return new Promise(function(resolve, reject) {
        _self._del(fileId, function(err) {
            if (err) {
                reject(err);
                return;
            }
...
FdfsClient.prototype.setMetaData = function(fileId, metaData, flag, callback) {
    ...
    if (callback) {
        this._setMetaData(fileId, metaData, flag, callback);
        return;
    }

    var _self = this;
    return new Promise(function(resolve, reject) {
        _self._setMetaData(fileId, metaData, flag, function(err) {
            if (err) {
                reject(err);
                return;
            }
...
FdfsClient.prototype.getMetaData = function(fileId, callback) {
    if (callback) {
        this._getMetaData(fileId, callback);
        return;
    }

    var _self = this;
    return new Promise(function(resolve, reject) {
        _self._getMetaData(fileId, function(err, meta) {
            if (err) {
                reject(err);
                return;
            }
...
FdfsClient.prototype.getFileInfo = function(fileId, callback) {
    if (callback) {
        this._getFileInfo(fileId, callback);
        return;
    }

    var _self = this;
    return new Promise(function(resolve, reject) {
        _self._getFileInfo(fileId, function(err, fileInfo) {
            if (err) {
                reject(err);
                return;
            }
